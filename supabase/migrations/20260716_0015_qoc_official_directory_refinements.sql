-- Keep QOC profile metrics aligned with the relations used by the Android app.
-- The denormalized profile counters are not maintained for legacy profiles.
  ...
  return jsonb_build_object(
    'items', coalesce((select jsonb_agg(row_data) from (
      select jsonb_build_object(
        ...
        'language', o.language, 'mediaUrl', o.media_url, 'mediaType', o.media_type,
