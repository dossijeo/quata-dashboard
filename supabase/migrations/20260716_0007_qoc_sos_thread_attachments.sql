-- Include message attachments in the restricted SOS conversation reader.
...
begin
  ...
  if not exists (select 1 from public.chat_sos_events e where e.thread_id = p_thread_id) then
    ...
    from (
      select
        ...
        jsonb_build_object(
          ...
          'isSos', exists (select 1 from public.chat_sos_events e where e.message_id = m.id)
            or m.body like '[SOS:%'
        ) as message
      from public.chat_messages m
      left join public.community_profiles p on p.id = m.sender_profile_id
      where m.thread_id = p_thread_id and m.deleted_at is null
