-- Historical production releases that predate the Google Play archive.
-- These notes are the approved store notes supplied by the Qüata team.
-- Alpha releases are intentionally excluded from the Android client history.

with release_data as (
    select *
    from (
        values
            (26::bigint, '1.0.0'::text),
            (27::bigint, '1.0.1'::text),
            (28::bigint, '1.0.2'::text)
    ) as release(version_code, version_name)
), upserted_releases as (
    insert into public.android_releases (
        package_name,
        track,
        version_code,
        version_codes,
    ...
    select
        ...
        array[release.version_code],
        release.version_name,
        'completed',
        true,
        3,
        jsonb_build_object('source', 'manual_release_history_backfill')
    from release_data release
    on conflict (package_name, track, version_code) do update
    set version_codes = excluded.version_codes,
        version_name = excluded.version_name,
        release_status = excluded.release_status,
        visible_in_whats_new = excluded.visible_in_whats_new,
